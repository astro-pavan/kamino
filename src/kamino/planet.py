...
class planet:
    ...
    def __init__(self, P_surface: float, ocean_depth: float, instellation: float, outgassing_rate: float, hydrothermal_flow_rate: float, seafloor_age: float, flow_path_length: float):
    ...
    def solve_climate_from_chemistry(self, t: float, Alk: float, C: float, Ca: float, T_init: float=288) -> tuple[float, float, float]:
        ...
        try:
            ...
            def T_s_residual(T_val):
                ...
                T_calc = get_T_surface(self.instellation, pco2, 0.05)
                return T_val - T_calc
    ...
    def solve_climate_from_CO2(self, P_CO2: float, T_init: float=288) -> tuple[float, float]:
        ...
        def T_s_residual(T_val):
            P_H2O = august_roche_magnus_formula(T_val) * 0.5
            T_calc = get_T_surface(self.instellation, P_CO2, 0.05)
            return T_val - T_calc
    ...
    def dY_dt(self, t, Y, full_precipitation_calculation=True, verbose=True):
        ...
        should_print = self.debug_counter % 1000
    ...
    def get_fluxes(self, t, Y, full_precipitation_calculation=True):
        ...
        if self.use_KT18_weathering:
            F_diss = get_weathering_rate_KT18(P_pore, T_pore, x_CO2) * self.surface_area # mol / s
        else:
            F_diss = get_weathering_rate(P_pore, T_pore, x_CO2, self.hydrothermal_flow_rate, PORE_DEPTH, self.seafloor_age) * self.surface_area # mol / s
        ...
        if full_precipitation_calculation:
            ...
            # rate_o, SI_o = get_calcite_data_fast(P_pore, T_seafloor, Ao, Co, Cao, S_area=1e-4)
            # rate_p, SI_p = get_calcite_data_fast(P_pore, T_pore, Ap, Cp, Cap, S_area=1e-4)
            ...
            # width = 1.0
            # switch_o = 0.5 * (np.tanh(SI_o / width) + 1)
            # ramp_o = np.clip(SI_o, 0, 1)
    ...
    def run_simulation(self, t_end, make_plots, Y0):
        ...
        # sol_t = np.hstack([sol1.t, sol2.t])

        # switch_time = sol1.t[-1]
        ...
        for t, T, P_CO2, Co, Ao, Cao in zip(results['time'], results['T_surface'], results['P_CO2'], results['C_ocean'], results['Alk_ocean'], results['Ca_ocean']):
            ...
            # F_diss_val = get_weathering_rate(P_pore, T_pore, x_CO2, self.hydrothermal_flow_rate, PORE_DEPTH, self.seafloor_age) * (self.surface_area)

            # # if t > switch_time:
            # #     F_prec_o = 0.5 * F_diss_val
            # # else:
            # rate_o, SI_o = get_calcite_precipitation_rate(P_pore, T_seafloor, Ao, Co, Cao)
    ...
    def get_weathering(self, P_CO2: float, use_KT18_weathering: bool=False, use_WHAK_weathering: bool=False, use_MAC_weathering: bool=False) -> float:
    ...
    def find_steady_state_no_evolution(self, use_KT18_weathering=False, use_WHAK_weathering=False, use_MAC_weathering=False, diagnostic_plots: bool=False) -> tuple[float, float, float]:
                                    
        def target_function_climate(pco2: float):

            weathering = self.get_weathering(pco2, use_KT18_weathering, use_WHAK_weathering, use_MAC_weathering)
            residual = weathering - self.outgassing
            # print(f'P_CO2 = {float(pco2):.2e} Pa  T_pore = {T_pore:.0f} K  Residual = {float(residual):.2e}')
