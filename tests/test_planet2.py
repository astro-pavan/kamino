...
def run_single_planet(params, rerun=False):
    """
    ...
    run_name = f"planet_inst_{inst:.2f}_tect_{tect:.4f}_depth_{depth:.1e}"
    run_dir = os.path.join(base_dir, run_name)

    # Check if the run already exists and is completed (look for output PNG file)
...
def run_parameter_sweep_tectonics():
    base_output_dir = "/data/pt426/sweep_results_tectonics_v2"
...
def run_parameter_sweep_ocean_depth():
    ...
    base_output_dir = "/data/pt426/sweep_results_depth"
    os.makedirs(base_output_dir, exist_ok=True)
...
def plot_steady_states(
    ...
    for file in json_files:
        with open(file, 'r') as f:
            ...
            pH = summary['final_state'].get('pH', None)

            P_CO2_bar = P_CO2_Pa / 1e5

            # Total ion concentration (salinity proxy): sum all elements except Alkalinity
            chem = summary['final_state'].get('ocean_chemistry_mol_kgw', {})
            total_conc = sum(v for k, v in chem.items() if k != 'Alkalinity')

            if param_value not in data_by_param:
                data_by_param[param_value] = {'inst': [], 'T': [], 'P_CO2': [], 'pH': [], 'salinity': []}
            ...
            data_by_param[param_value]['pH'].append(pH)
            data_by_param[param_value]['salinity'].append(total_conc)

    # 2. Setup the plot
    n_panels = 2 + int(show_pH) + int(show_salinity)
    fig_height = 4 + 2 * n_panels
    fig, axes = plt.subplots(n_panels, 1, figsize=(8, fig_height), sharex=True)
    ax1, ax2 = axes[0], axes[1]
    panel_idx = 2
    ax3 = axes[panel_idx] if show_pH else None
    if show_pH:
        panel_idx += 1
    ax4 = axes[panel_idx] if show_salinity else None
    ...
    for val in sorted(param_values):
        ...
        if show_pH:
            pH_vals = data_by_param[val]['pH']
            if any(v is not None for v in pH_vals):
                pH_sorted = np.array(pH_vals, dtype=float)[sort_idx]
                ax3.plot(inst_sorted, pH_sorted, color=color, linewidth=1.5)

        if show_salinity:
            sal_sorted = np.array(data_by_param[val]['salinity'])[sort_idx]
            ax4.plot(inst_sorted, sal_sorted, color=color, linewidth=1.5)
    ...
    # 5. Formatting Middle Panel (P_CO2)
    ...
    # 6. Formatting pH panel if requested
    if show_pH:
        ax3.set_ylabel('Ocean pH', fontsize=12)
        ax3.tick_params(axis='both', which='major', labelsize=10)
        ax3.grid(True, linestyle='--', alpha=0.5)

    # 7. Formatting salinity panel if requested
    if show_salinity:
        ax4.set_ylabel('Total ion conc.\n(mol kg$^{-1}$)', fontsize=12)
        ax4.set_yscale('log')
        ax4.tick_params(axis='both', which='major', labelsize=10)
        ax4.grid(True, linestyle='--', alpha=0.5)

    # Set x-label on the bottom panel
    axes[-1].set_xlabel('Instellation ($S/S_0$)', fontsize=12)
...
if __name__ == "__main__":
    ...
    # plot_steady_states(
    #     results_dir="/data/pt426/sweep_results_tectonics_v2",
    #     sweep_param_key="tectonics",
    #     cbar_label="Tectonics Factor",
    #     cmap_name="inferno",
    #     output_filename="steady_states_tectonics.pdf",
    #     show_pH=True,
    #     show_salinity=True
    # )

    # plot_steady_states(
    #     results_dir="/data/pt426/sweep_results_depth",
