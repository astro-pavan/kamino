...
def run_single_planet(params, rerun=True):
    ...
    """
    inst, tect, base_dir = params
    run_name = f"planet_inst_{inst:.2f}_tect_{tect:.2f}"
...
def run_parameter_sweep_mp():
    ...
    print(combinations)
    tasks = [(inst, tect, base_output_dir) for inst, tect in combinations]
...
def plot_steady_states():
    # 1. Locate all summary JSON files from the sweep
    search_pattern = os.path.join("sweep_results", "**", "*_summary.json")
    json_files = glob.glob(search_pattern, recursive=True)
    
    if not json_files:
        print("No summary files found. Make sure you are running this in the same directory as 'sweep_results'.")
        return
    ...
    for file in json_files:
        with open(file, 'r') as f:
            ...
            tect = summary['input_parameters']['tectonics']
            T_final = summary['final_state']['T_K']
            P_CO2_Pa = summary['final_state']['P_CO2_Pa']
            
            P_CO2_bar = P_CO2_Pa / 1e5
            
            if tect not in data_by_tectonics:
                data_by_tectonics[tect] = {'inst': [], 'T': [], 'P_CO2': []}
                
            data_by_tectonics[tect]['inst'].append(inst)
            data_by_tectonics[tect]['T'].append(T_final)
            data_by_tectonics[tect]['P_CO2'].append(P_CO2_bar)
    ...
    tect_values = list(data_by_tectonics.keys())
    norm = colors.LogNorm(vmin=min(tect_values), vmax=max(tect_values))
    
    # 3. Plot the lines
    for tect in sorted(tect_values):
        sort_idx = np.argsort(data_by_tectonics[tect]['inst'])
        inst_sorted = np.array(data_by_tectonics[tect]['inst'])[sort_idx]
        T_sorted = np.array(data_by_tectonics[tect]['T'])[sort_idx]
        P_CO2_sorted = np.array(data_by_tectonics[tect]['P_CO2'])[sort_idx]
        
        color = cmap(norm(tect))
    ...
    # Add Snowball Region (Blue, below freezing ~273.15K) [cite: 12]
    ax1.axhspan(240, 273.15, color='#ccccff', alpha=0.5, zorder=0)
    ax1.text(0.25, 260, 'Snowball', fontsize=12, va='center', ha='left', zorder=3)
    
    # Add Runaway Greenhouse Region (Red, >= 340K based on your reference) [cite: 9, 3]
    ...
    # Lock the y-axis to match the visual frame of the reference [cite: 2, 3, 11]
    ...
    # Lock the P_CO2 limits to match the reference [cite: 1, 13, 16]
    ...
    cbar.set_label('Tectonics Factor', fontsize=12)
