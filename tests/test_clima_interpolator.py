...
S_range = np.linspace(0.3, 1.3, num=100) * SOLAR_CONSTANT
...
for p_co2 in p_co2_range:
    for S in S_range:
        T_s = get_T_surface(S, p_co2, 0.22)
...
y = np.array(p_co2_arr)
x = np.array(S_arr) / SOLAR_CONSTANT
z = np.array(T_arr)
...
ax.scatter([1], [40], s=10, c='black', label='Earth')
...
# Axis Labels
ax.set_ylabel('$P_{CO2}$')
ax.set_xlabel('S ($S_{\\oplus}$)')
